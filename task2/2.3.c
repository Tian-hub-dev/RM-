...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void Set_motor(uint8_t dir,uint16_t speed)
{
...
void calculate_speed(void)
{
    ...
    if(current_time - last_time >= time)
    {
        ...
        motor_speed_rpm = (pulse * 60) / (encoder) ;
        pulse  = 0; 
...
Set_motor(mode_dir, first_speed );
/* USER CODE END 2 */
  /* USER CODE END 2 */
  ...
  while (1)
  {
		  ...
		  if(HAL_GPIO_ReadPin (KEY1_GPIO_Port ,KEY1_Pin )==GPIO_PIN_SET )
	  {
		  ...
		  uint16_t  current_speed=(mode_speed ==0)? low_speed :high_speed ;
		  Set_motor (mode_dir,current_speed);
...
//        HAL_TIM_ReadCapturedValue(&htim3, TIM_CHANNEL_2);        //清除中断标志位，“1”(中断正在进行）->"0"
